...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_dim = 1024   # mlp的定义是为了统一输入输出的维度？
        self.h_dim = h_dim
        ...
        self.spatial_embedding = nn.Linear(2, embedding_dim)# 2维的输入，64维的输出
    ...
    def forward(self, obs_traj):
        ...
        obs_traj_embedding = self.spatial_embedding(obs_traj.view(-1, 2))# 2维的输入，64维的输出
        obs_traj_embedding = obs_traj_embedding.view(
            -1, batch, self.embedding_dim
        )# 维数变换，（-1, 64）->（-1, batch, 64）
...
class PoolHiddenNet(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_embedding = nn.Linear(2, embedding_dim)
        self.mlp_pre_pool = make_mlp(       # mlp_pre_pool是为了对相对位置进行编码
            mlp_pre_pool_dims,      # [mlp_pre_dim, 512, bottleneck_dim]
            activation=activation,
            batch_norm=batch_norm,
            dropout=dropout)
    ...
    def repeat(self, tensor, num_reps):
        ...
        """     
        col_len = tensor.size(1)            # tensor.size(1)是tensor的列数
        # 在第1维上增加一个维度，然后在第1维上重复num_reps次，
        tensor = tensor.unsqueeze(dim=1).repeat(1, num_reps, 1)
        tensor = tensor.view(-1, col_len) # 最后将tensor的维度变为(-1, col_len)
        return tensor
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            num_ped = end - start  # 行人数量
            curr_hidden = h_states.view(-1, self.h_dim)[start:end] 
            curr_end_pos = end_pos[start:end]   # 当前帧的绝对位置
            # Repeat -> H1, H2, H1, H2
            # 当前所有行人的位置，重复是为了对场景中所有人进行并行处理
            curr_hidden_1 = curr_hidden.repeat(num_ped, 1)
            # Repeat position -> P1, P2, P1, P2
            # 当前所有行人位置，（被关注行人自身和周围行人）
            curr_end_pos_1 = curr_end_pos.repeat(num_ped, 1) # 使用pytorch中的repeat函数， 每行重复num_ped次，每列重复1次
            # Repeat position -> P1, P1, P2, P2
            # 当前所有行人位置，（被关注行人自身）
            curr_end_pos_2 = self.repeat(curr_end_pos, num_ped) # 使用自定义的repeat函数，每行重复num_ped次，每列重复1次
            # 计算周围行人到被关注行人的相对位置，所有的repeat都是为了对场景中所有人进行并行处理
            curr_rel_pos = curr_end_pos_1 - curr_end_pos_2 #(p1-p1, p1-p2, p2-p1, p2-p2)
            # 对相对位置进行编码
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)   (8, 1, 2)
        - obs_traj_rel: Tensor of shape (obs_len, batch, 2)     (8, 1, 2)
        - seq_start_end: A list of tuples which delimit sequences within batch.
        - user_noise: Generally used for inference when you want to see
        relation between different types of noise and outputs.
        Output:
